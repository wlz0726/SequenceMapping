...
# 2017.8
# remove reads in the following ways:
# 1) >=10% unidentified nucleotides (N)
# 2) with a phred quality <=7 for >65% of the read length
# 3) reads are trimmed if they had three consecutive bp with a phred quality of <=13, and discarded if they were shorter than 45 bp
...
########################################### 3bp sliding window, two side trim mean qual < 13; after trim, remove reads shorter than 45 bp
sub trimming{
    my ($seq,$qual)=@_;
    chomp $seq;
    chomp $qual;
